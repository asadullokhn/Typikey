...
- Home is the generative board: all of Core — pronouns (I, you, he, she, it, we, they, me, my), the auxiliaries `be`, `do`, `have`, the verbs that combine with everything, and the closed classes that actually build sentences: `to for with in on at of from out up`, `and but or because`, `a the`. That is where the freedom is. A stored phrase says one thing; "I am waiting" is a dead end without `for`, and no amount of prediction fixes that — a preposition has to sit in one known place, one tap away, every time. Core word lists (Banajee et al.; Boenisch & Soto) rank these among the highest-frequency words in everything anyone says, which is why every published core board carries them permanently. Fixed phrases stay one tap away on Chat
...
- Three keyboard sizes, chosen in the app rather than on the board — a grid slot is a word, and size is set once and then never again. Rows follow the size, so keys never shrink: Large is six rows of ten at roughly 97pt each (taller than a key on the system keyboard), Medium five, Small four
...
- Grammar support, in both the forms the AAC products offer it:
  ...
  - **User-driven** — a tense key cycles now / past / future and every verb key follows it: `go` → `went` → `will go`, `be` → `was`/`were` → `will be`. Tense is the user's choice because no context can supply it ("you" says who, never when), and it resets at the end of each sentence so a past-tense sentence can't quietly corrupt the next one. This is the same mechanism CoughDrop/Sweet Suite calls an inflection shift and TD Snap calls a grammar bar
